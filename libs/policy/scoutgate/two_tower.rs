...
impl TwoTowerScorer {
    ...
    pub fn new(d_context: usize, d_prime: usize, device: Device) -> Result<Self, ScoutGateError> {
    ...
    pub fn score_experts(
        ...
        let biased_scores = if let Some(bias) = self.layer_biases.get(&layer_id) {
            let expanded_bias = bias
                .clone()
                .unsqueeze::<2>()
                .expand([raw_scores.shape().dims[0], bias.shape().dims[0]]);
