...
fn test_complete_scoutgate_system() {
    ...
    for (i, token) in test_tokens.iter().enumerate() {
        predictor.update_token_context(*token)
            .expect(&format!("Failed to update token context with token {}", token));
    ...
    let layer_0_scores = predictor.predict_layer(0)
        .expect("Failed to predict layer 0");
    
    println!("✅ Layer 0 prediction: {} expert scores", layer_0_scores.len());
    ...
    for (layer_id, layer_probs) in probabilities.inner.iter().enumerate() {
        assert_eq!(layer_probs.len(), 16, "Layer {} should have 16 experts", layer_id);
    }
    ...
    println!("✅ Probability validation: {}/{} predictions valid", valid_predictions, total_predictions);
    println!("✅ Average probability: {:.4}", sum_probabilities / valid_predictions as f64);
    ...
    let layer_1_scores_1 = predictor.predict_layer(1)
        .expect("Failed to predict layer 1 (first call)");
    let layer_1_scores_2 = predictor.predict_layer(1)
        .expect("Failed to predict layer 1 (second call)");
    
    // Verify consistency (scores should be identical for same input)
    for (i, (&score1, &score2)) in layer_1_scores_1.iter().zip(layer_1_scores_2.iter()).enumerate() {
        let diff = (score1 - score2).abs();
        assert!(diff < 1e-6, "Inconsistent scores for expert {}: {} vs {}", i, score1, score2);
    }
    ...
    assert!(invalid_layer_result.is_err(), "Should fail for invalid layer ID");
...
fn test_scoutgate_performance_characteristics() {
    ...
    let start = std::time::Instant::now();
    for layer_id in 0..5 { // Test first 5 layers for performance
        predictor.predict_layer(layer_id)
