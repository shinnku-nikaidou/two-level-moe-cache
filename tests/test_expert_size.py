...
class TestExpertSizes:
    ...
    def calculate_expert_size_precise(self, weights, model_name="gpt-oss-20b"):
        ...
        for name, tensor in layer_0_mlp_params.items():
            ...
            print(
                f"  {name:30}: {str(shape):20} ({numel:>12,} params, {size_mb:>8.2f} MB)"
            )
        ...
        print(f"  {'='*30} {'='*20} {'='*15} {'='*12}")
        print(
            f"  {'TOTAL LAYER':30}: {'':<20} ({total_layer_params:>12,} params, {total_layer_mb:>8.2f} MB)"
        )
        ...
        if expert_count:
            ...
            print(
                f"  Size per expert: {expert_size_mb:.2f} MB ({expert_params:,} params)"
            )
    ...
    def validate_rust_assumptions(self, expert_info):
        ...
        for model, assumed_bytes in rust_assumptions.items():
            ...
            if "gpt-oss-20b" in model.lower():  # Only validate for current model
                ...
                print(
                    f"  Actual vs assumption: {expert_size_mb:.2f} MB vs {assumed_mb:.2f} MB"
                )
                ...
                if abs(ratio - 1.0) < 0.1:
                    ...
                    if ratio > 1.5:
                        print(
                            f"      Rust assumption too low, increase to ~{expert_size_bytes}"
                        )
                    else:
                        print(
                            f"      Rust assumption too high, decrease to ~{expert_size_bytes}"
                        )
    ...
    def calculate_memory_scenarios(self, expert_info):
        ...
        for scenario_name, expert_count in cache_scenarios:
            if expert_count <= total_experts:
                ...
                print(
                    f"  {scenario_name:8} ({expert_count:3} experts): {cache_gb:5.1f} GB ({percent:4.1f}% of model)"
                )
    ...
    def test_gpt_oss_20b_expert_size(self):
        ...
        if not model_path.exists():
            ...
            print(
                "Please ensure GPT-OSS 20B model is downloaded to data/models/gpt-oss-20b/original/"
            )
            return None
    ...
    def test_gpt_oss_120b_expert_size(self):
        ...
        if not model_path.exists():
            print(f"Model path not found: {model_path}")
            print(
                "Please ensure GPT-OSS 120B model is downloaded to data/models/gpt-oss-120b/original/"
            )
            return None
    ...
    def test_compare_models(self):
        ...
        if len(results) > 1:
            ...
            for model_name, result in results.items():
                print(
                    f"  {model_name:15}: {result['expert_size_mb']:6.1f} MB per expert"
                )
