...
class LazyExpertTensor:
    ...
    def __init__(
        ...
        # Use the target device directly, no need to force CPU
        self.checkpoint = Checkpoint(checkpoint_path, device)
    ...
    def load_experts(self, expert_indices: torch.Tensor) -> torch.Tensor:
        ...
        # expert_indices can be on any device, PyTorch handles indexing automatically
...
class LazyMLPBlock(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        try:
            # Load only the selected expert weights (no need to force CPU)
